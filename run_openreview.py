...
def main(args):
    ...
    collection = client.create_collection(
        name="ollama-paper-bot", metadata={"hnsw:space": "cosine"}
    )
    ...
    for result in data["results"]:
        ...
        collection.add(
            ids=title[0]["text"]["content"], embeddings=embedding, documents=url
        )
    ...
    for venue in tqdm(
        openreview.fetch_venues(conference, year),
        ...
        for paperinfo in openreview.fetch_invitation_paper(invitation, True):
            if paperinfo.abstract == "":
                continue
            embedding: np.ndarray = get_embedding(paperinfo.abstract, embed_model)
            if len(embedding) == 0:
                raise ValueError(paperinfo)
    ...
    channel_id = "C04J600JK18"  # os.getenv("CHANNEL_ID")
    ...
    for query_idx in query_idxs:
        ...
        attachment["fields"].append(
            {"title": "Authors", "value": ", ".join(query_info.authors), "short": False}
        )
        for i, key_idx in enumerate(
            np.where(cosine_similarty[query_idx] > args.threshold)[0]
        ):
    ...
    for attachment in attachment_list:
        time.sleep(0.1)
        client.post_thread_message(channel_id, result["ts"], attachments=[attachment])
